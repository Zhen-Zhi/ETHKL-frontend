...
const PostScreen: React.FC = () => {
  ...
  const restaurantIds = [0n, 1n, 2n, 3n]; // Assuming there are exactly 4 restaurants

  // Map over restaurantIds and create the query for each one
  ...
  const submitReview = () => {
    ...
    if (selectedRestaurantId === null || newReview.text === '' || image == null || image?.length === 0 ) {
      ...
      <ScrollView className='flex-1 p-4 bg-slate-100'>
        ...
        {restaurants.map(restaurant => (
          <AnimatedPressable 
            ...
            pressInValue={0.98} 
            key={restaurant?.data?.[0]}
            onPress={() => {
              setSelectedRestaurantId(Number(restaurant?.data?.[0])); // Save the selected restaurant ID
          ...
          >
            ...
            <View className='mx-3 p-2'>
              <Text className='text-xl font-bold'>{restaurant.data?.[1]}</Text>
              <Text className=''>Descriptions goes here.</Text>
            </View>
          </AnimatedPressable>
        ))
        }
